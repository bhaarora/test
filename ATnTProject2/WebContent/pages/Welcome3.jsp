...
<form action="/ATnTProject/addProduct" method="post" id="my_form" enctype = "multipart/form-data">
...
<input type="text" id="prodDesc" name="prodDesc" placeholder="product Desc" >
...
<c:forEach var="list" items="${requestScope.productList}">
       ...
       <td>${list.prodImageLink}</td>
        <td><img src="${list.imageUrl}" alt="Smiley face" width="42" height="42"></td>
...
$(this).validate({
			...
			messages: {
				prodId: "Please enter your Product Id ",
				prodName: "Please enter your Product Name",
				prodDesc: {
       ...
       $.ajax({
        ...
        var obj = JSON.parse(response);
         $("#server-results").html("Product with Product Id "+obj.prodId+" added successfully" );
         var prodImage = "<td><img src='"+obj.imageUrl+"' alt='Smiley face' width='42' height='42'></td>";
        var updateLink ="<td><a href='/ATnTProject/updateProduct?key="+obj.prodId+"'>Update Product</a> </td></tr>";
        var rowString = "<tr id='row"+obj.prodId+"'><td><input type='checkbox' name='record' value ='"+obj.prodId+"' ></td><td>" + obj.prodId + "</td><td>" + obj.prodName + "</td><td>" + obj.prodDesc +  "</td><td>"  + obj.relProd +  "</td><td>" + obj.prodImageLink + "</td>"+prodImage+updateLink;
         $("#row"+obj.prodId).remove();
        $("table tbody").append(rowString);
